    ...
    return {
        ...
        makeUploadDataStructure(upload) {
            ...
            tokens.forEach((token, rowCount) => {
                if (BreakdownParse.rowHasScope(token, 'issue.epic.jira')) {
                    ...
                    issue = this.makeUploadSubTaskIssue(token, upload.settings, (parentStoryIssue ? parentStoryIssue : parentEpicIssue))
                    upload.newJiraObjects.push({
                        issue: issue,
                        parent: (parentStoryIssue ? parentStoryIssue : parentEpicIssue)
                    });
        ...
        maintainParent(uploadJiraObject, settings) {
            ...
            if (BreakdownUtils.isStoryType(issue, settings)) {
                this.maintainEpicLink(issue, uploadJiraObject.parent, settings);
            } else if (BreakdownUtils.isSubTaskType(issue, settings)) {
                this.maintainParentLink(issue, uploadJiraObject.parent, settings);
            }
        },
