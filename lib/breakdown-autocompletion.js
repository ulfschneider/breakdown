...
export default class BreakdownAutocompletionProvider {
    ...
    pullComponents(query) {
        if (BreakdownUtil.isFalsy(this.components) &&
            !this.isOffline() &&
    ...
    pullVersions(query) {
        if (BreakdownUtil.isFalsy(this.versions) &&
            !this.isOffline() &&
            this.prepareAutocompletion()) {
    ...
    pullTransitions(key, query) {
        ...
        if (BreakdownUtil.isFalsy(transitions) &&
            !this.isOffline() &&
            this.prepareAutocompletion()) {
            let options = BreakdownRequest.prepareOptions({
    ...
    mapSuggestions(suggestions, query) {
        if (BreakdownUtil.isFalsy(suggestions)) {
            return [];
        }
