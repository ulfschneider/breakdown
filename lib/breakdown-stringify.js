    ...
    return {
        ...
        stringifySettings(download) {
            ...
            if (download.settings.fields.length > 0) {
                result += 'fields: ';
                result += BreakdownUtil.concatString(download.settings.fields, ' ');
                result += lineEnding;
            }
                    ...
                    .forEach((assignee) => {
                        ...
                        if (issueCount) {
                            ...
                            if (unresolvedCount) {
                                ...
                                result += ' unresolved: '
                                result += BreakdownUtil.concatString(unresolvedIssues);
                            }
                            result += lineEnding;
                        }
                    });
        ...
        hasField(settings, field) {
            return _.find(settings.fields, (f) => {
                return BreakdownUtil.equalsIgnoreCase(f, field);
            });
        },
        ...
        hasAssignee(issue, settings) {
            return this.hasField(settings, 'assignee') && BreakdownUtil.getAssignee(issue);
        },

        hasPoints(issue, settings) {
            return this.hasField(settings, 'points') && BreakdownUtil.getStoryPoints(issue, settings);
        },
        ...
        summarizePoints(jiraObject, settings, onlyDonePoints) {
            ...
            if (issue && this.hasPoints(issue, settings)) {
                if (BreakdownUtil.isFalseOrUndefined(onlyDonePoints) ||
