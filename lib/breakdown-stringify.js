...
export default BreakdownStringify = (function () {
    ...
    return {
        ...
        stringifyDescription(jiraObject) {
            let result = ''; 
            let issue = jiraObject.issue;
            ...
            if (description.length) {
                for(let line of description) {
                    result += '\t\t\t// ';
                    result += BreakdownUtil.trim(line);
        ...
        stringifyJiraObject(jiraObject) {
            ...
            if (issue) {
                ...
                result += BreakdownEnvironment.getEditorLineEnding();
                result += this.stringifyDescription(jiraObject);
            }
            return result;
        },
        ...
        stringifyFields(jiraObject) {
            ...
            if (this.hasPrimaryFields(issue) || totalPoints) {
                ...
                if (this.hasOriginalEstimate(issue)) {                    
                    let origEstimate = BreakdownUtil.getOriginalEstimate(issue);
                    result += ' o:' + origEstimate; 
                }
        ...
        hasOriginalEstimate(issue) {
            return BreakdownUtil.getOriginalEstimate(issue);
        },
