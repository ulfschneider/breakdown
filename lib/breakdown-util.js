...
export default BreakdownUtil = (function() {
    ...
    return {
        ...
        deleteStatus(issue) {
            this.setStatus(issue, null);
        },


        setStoryPoints(issue, points, settings) {
            if (settings.storyPointsFieldName) {
                let i = this.parseInteger(points);
                issue.fields[settings.storyPointsFieldName] = i ? i : null;
            }
        },
        ...
        setFixVersion(issue, version) {
            let versionArray = version.split(',');
            this.setFixVersions(issue, versionArray);
        },
