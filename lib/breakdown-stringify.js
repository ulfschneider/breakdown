...
export default BreakdownStringify = (function() {
    ...
    return {
        ...
        isInActiveSprint(jiraObject, settings) {
            let issue = jiraObject.issue;
            if (issue && BreakdownUtil.isInActiveSprint(issue, settings)) {
                return true;
        ...
        stringifyActiveSprint(jiraObject, settings) {
            if (this.isInActiveSprint(jiraObject, settings)) {
        ...
        stringifyJiraObject(jiraObject, settings) {
            ...
            if (issue) {
                ...
                result += this.stringifyActiveSprint(jiraObject, settings);
        ...
        getWorkingHoursPerDay(settings) {
        ...
        getWorkingDaysPerWeek(settings) {
        ...
        stringifyTimeTracking(timeSpentSeconds, remainingEstimateSeconds, settings) {
            let result = '';
            let spentTime = BreakdownUtil.stringifySeconds(timeSpentSeconds,
                this.getWorkingHoursPerDay(settings),
                this.getWorkingDaysPerWeek(settings));
        ...
        stringifyUnresolvedIssues(assignee, settings) {
            let unresolvedIssues = assignee.getUnresolvedIssues();
            let lineEnding = BreakdownUtil.getEditorLineEnding();
            let result = '\t\t';
            result += unresolvedIssues.length;
            result += ' issues unresolved';
            let timeTracking = this.stringifyTimeTracking(assignee.getUnresolvedTimeSpentSeconds(), assignee.getUnresolvedRemainingEstimateSeconds(), settings);
            if (timeTracking) {
                result += ' ';
                result += timeTracking;
            }
            ...
            unresolvedIssues.forEach((issue, index) => {
                result += '\t\t';
                if (BreakdownUtil.isInActiveSprint(issue, settings)) {
                ...
                result += ', ';
                result += BreakdownUtil.getStatus(issue);
                let timeTracking = this.stringifyTimeTracking(BreakdownUtil.getTimeSpentSeconds(issue), BreakdownUtil.getRemainingEstimateSeconds(issue), settings);
                if (timeTracking) {
                    result += ' ';
                    result += timeTracking;
                }
        ...
        stringifyResolvedIssues(assignee, settings) {
            let resolvedIssues = assignee.getResolvedIssues();
            let lineEnding = BreakdownUtil.getEditorLineEnding();
            let result = '\t\t';
            result += resolvedIssues.length;
            result += ' issues resolved';
            let timeTracking = this.stringifyTimeTracking(assignee.getResolvedTimeSpentSeconds(), assignee.getResolvedRemainingEstimateSeconds(), settings);
            if (timeTracking) {
                result += ' ';
                result += timeTracking;
            }
        ...
        stringifyDownloadStatistics(download) {
            ...
            if (assigneeCount) {
                ...
                let timeTracking = this.stringifyTimeTracking(download.assignees.getTimeSpentSeconds(),
                    download.assignees.getRemainingEstimateSeconds(), settings);
                ...
                download.assignees
                    .forEach(assignee => {
                        ...
                        let timeTracking = this.stringifyTimeTracking(assignee.getTimeSpentSeconds(),
                            assignee.getRemainingEstimateSeconds(), settings);

                        if (issueCount) {
                            result += '\t';

                            if (BreakdownUtil.hasActiveSprint(assignee.getIssues(), settings)) {
                                result += BreakdownUtil.ACTIVE_SPRINT;
                            }
                            ...
                            if (unresolvedCount) {
                                result += lineEnding;
                                result += this.stringifyUnresolvedIssues(assignee, settings);
                            }
                            if (resolvedCount) {
                                result += lineEnding;
                                result += this.stringifyResolvedIssues(assignee, settings);
                            }
                            result += lineEnding;
                        }
                    });
        ...
        stringifyJiraContent(download) {
            ...
            for (let epicKey of download.epics.keys()) {
                ...
                if (epicKey == BreakdownUtil.NO_PARENT_IN_SELECTION_KEY && (epic.stories.size || epic.subTasks.size)) {
                    result += this.stringifyActiveSprint(epic, download.settings);
