    ...
    return {
        ...
        beautifyRow(token, rowCount) {
            ...
            for (let i = 0; i < token.length; i++) {
                ...
                if (BreakdownParse.tokenHasScope(t, 'issue.delete.jira')) {
                ...
                } else if (BreakdownParse.tokenHasScope(t, 'empty-parent.jira')) {
                    row += BreakdownUtil.EMPTY_PARENT;
                } else if (BreakdownParse.tokenHasScope(t, 'no-parent-in-selection.jira')) {
                    ...
                    row += settings.query;
                } else if (BreakdownParse.tokenHasScope(t, 'settings.options.jira') && !BreakdownParse.tokenHasScope(t, 'settings.punctuation.separator.key-value.jira')) {
                    if (BreakdownParse.tokenHasScope(t, 'settings.key.jira')) {
                        row += 'options';
                    } else if (BreakdownParse.tokenHasScope(t, 'settings.value.jira')) {
                        row += value;
                    } else {
                        continue;
                    }
