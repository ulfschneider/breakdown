    ...
    return {
        ...
        displayStatistics(upload) {
            ...
            if (upload.statistics.failed.create.length ||
                ...
                let failed = 'Failures when pushing to JIRA';
                if (upload.statistics.failed.create.length) {
                    failed += '\n\nIssues not created\n';
        ...
        isNoPushDelete(settings) {
            return BreakdownUtil.hasOption(settings, 'nopushdelete');
        },

        isNoPushUpdate(settings) {
            return BreakdownUtil.hasOption(settings, 'nopushupdate');
        },

        isNoPushCreate(settings) {
            return BreakdownUtil.hasOption(settings, 'nopushcreate');
        },
        ...
        deleteIssue(deleteJiraObject, upload, credentials) {
            ...
            if (this.isNoPushDelete(upload.settings)) {
                upload.statistics.failed.delete.push(BreakdownUtil.createIssueMarkupLink(BreakdownUtil.getKey(issue), settings) + ' due to **nopushdelete** option');
                console.log('Issue could not be deleted in JIRA, key=[' + BreakdownUtil.getKey(issue) + '] due to **nopushdelete** option');
        ...
        updateIssue(updateJiraObject, upload, credentials) {
            ...
            if (this.isNoPushUpdate(upload.settings)) {
                upload.statistics.failed.update.push(BreakdownUtil.createIssueMarkupLink(BreakdownUtil.getKey(issue), settings) + ' due to **nopushupdate** option');
                console.log('Issue could not be updated in JIRA, key=[' + BreakdownUtil.getKey(issue) + '] due to **nopushupdate** option');
                return Promise.resolve();
            }
        ...
        createIssue(uploadJiraObject, upload, credentials) {
            ...
            if (this.isNoPushCreate(upload.settings)) {
                upload.statistics.failed.create.push(BreakdownUtil.getIssueType(issue) + ' ' + BreakdownUtil.getSummary(issue) + ' due to **nopushcreate** option');
                console.log('Issue could not be created in JIRA, issuetype=[' + BreakdownUtil.getIssueType(issue) + '] summary=[' + BreakdownUtil.getSummary(issue) + '] due to **nopushcreate** option');
                return Promise.resolve();
            }
