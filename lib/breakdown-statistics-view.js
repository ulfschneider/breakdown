...
export default class BreakdownStatisticsView {
    ...
    slideUp(callback) {
        let img = this.chartCache.get('CFD' + BreakdownEnvironment.getActivePath());

        if ((BreakdownEnvironment.isHandlingDifferentEditors() && BreakdownUtil.isFalsy(img)) ||  !BreakdownEnvironment.isBreakdownGrammar()) {
    ...
    renderCumulativeFlowDiagram() {
        ...
        settings.data = BreakdownEnvironment.getTransitionLog().transitionsToJSON({
        ...
        this.appendPrediction(settings);
    ...
    renderDurationDiagram(settings) {
        ...
        settings.data = BreakdownEnvironment.getTransitionLog().durationsToJSON({
            fromDate: settings.fromDate,
            toDate: settings.toDate,
        });
        ...
        settings.z = d3.scaleLinear()
            .range([settings.height, 0]);
        settings.y.domain([0, d3.max(settings.data, function(d) {
            return 1; //TODO sum of in progress items / points
        })]);
        ...
        this.appendLines(settings);
        this.appendDurationsAxis(settings);
        //TODO this.appendLegend(settings);
        this.appendMarkers(settings);
    ...
    appendLegend(settings) {
        ...
        let fileName = settings.g.append('text')
            ...
            .style('text-anchor', 'start')
            .style('fill', settings.newStyle.color)
            .text(BreakdownEnvironment.getFileNameWithoutExtension() +
                ' at ' +
