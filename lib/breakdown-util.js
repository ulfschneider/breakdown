...
export default BreakdownUtil = (function() {
    ...
    return {
        ...
        createIssueMarkupLink(key, settings) {
            let link = '';
            if (_.isArray(key)) {
                let keys = [];
                key.forEach(k => {
                    keys.push(this.createIssueMarkupLink(k, settings));
                });
                link = this.concat(keys, ', ');
            } else {
                if (key && settings) {
                    link = '[' + key + '](' + settings.jiraUrl + '/browse/' + key + ')';
                }
            }
            return link;
        },
        ...
        isInActiveSprint(issue, settings) {
        ...
        hasActiveSprint(issues, settings) {
            for (let issue of issues) {
                if (this.isInActiveSprint(issue, settings)) {
                    return true;
                }
            }
            return false;
