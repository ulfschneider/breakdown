    ...
    return {
        ...
        getIssueDiff(originIssue, modifiedIssue, modifiedParentIssue) {
            ...
            if (!BreakdownUtil.isSubTaskType(modifiedIssue)) {
                //do not maintain fixversions for sub-tasks
            ...
            let modifiedAcceptanceCriteria = BreakdownUtil.getAcceptanceCriteria(modifiedIssue);
            let originAcceptanceCriteria = BreakdownUtil.getAcceptanceCriteria(originIssue);
            if (BreakdownUtil.joinLines(originAcceptanceCriteria) !== BreakdownUtil.joinLines(modifiedAcceptanceCriteria)) {
                BreakdownUtil.setAcceptanceCriteria(diffIssue, modifiedAcceptanceCriteria, originIssue);
            }
            ...
            if (!_.isEmpty(diffIssue.fields)) {
                ...
                console.log(`Modified fields for ${BreakdownUtil.getKey(modifiedIssue)}\n${JSON.stringify(diffIssue)}`);
                BreakdownUtil.setIssueType(diffIssue, modifiedIssueType, BreakdownUtil.METHOD_UPDATE);
        ...
        injectUploadDescription(token, issue, descriptionType) {
            let description = BreakdownParse.parseDescription(token);
            if (descriptionType != ACCEPTANCE) {
                BreakdownUtil.addDescription(issue, description);
            } else {
                BreakdownUtil.addAcceptanceCriteria(issue, description);
            }
        ...
        makeUploadDataStructure() {
            ...
            tokens.forEach((token, rowCount) => {
                ...
                if (!BreakdownParse.rowHasScope(token, 'issue.description.field.bkdn') &&
                    !BreakdownParse.rowHasScope(token, 'issue.acceptance.field.bkdn') &&
                    !BreakdownParse.rowHasScope(token, 'description.value.bkdn')) {
                    descriptionType = DESCRIPTION;
                } else if (BreakdownParse.rowHasScope(token, 'issue.description.field.bkdn')) {
                    descriptionType = DESCRIPTION;
                } else if (BreakdownParse.rowHasScope(token, 'issue.acceptance.field.bkdn')) {
                    descriptionType = ACCEPTANCE;
                    ...
                    this.injectUploadDescription(token, parentIssue, descriptionType);
                }
            });
        ...
        pushIssues() {
            ...
            return upload.uploadJiraObjects.reduce((series, uploadJiraObject, count) => {
                return series.then(() => {
                    BreakdownStatusBar.push('Pushing  ' +
                        BreakdownStringify.stringifyProgress(count + 1, upload.uploadJiraObjects.length) + ' ' + BreakdownEnvironment.getFileNameWithoutExtension());
        ...
        pushToJira() {
            let settings = BreakdownEnvironment.getSettings();
            BreakdownStatusBar.push('Pushing ' + BreakdownEnvironment.getFileNameWithoutExtension());
