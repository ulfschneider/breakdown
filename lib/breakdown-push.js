...
export default BreakdownPush = (function() {
    ...
    return {
        ...
        addStoryKeyToEpic(upload, epicIssue, storyIssue) {
            let epicKey = BreakdownUtil.getKey(epicIssue);
            if (!epicKey) {
                epicKey = 'NO PARENT';
            }

            upload.epics.get(epicKey)
                .storyKeys.push(BreakdownUtil.getKey(storyIssue));
        },

        addEpic(upload, epicIssue) {
            upload.epics.set(BreakdownUtil.getKey(epicIssue), {
                storyKeys: []
            });
        },
        ...
        makeUploadDataStructure(upload, download) {
            ...
            upload.epics = new Map();
            upload.epics.set('NO PARENT', {
                storyKeys: []
            });
            ...
            tokens.forEach((token, rowCount) => {
                ...
                if (BreakdownParse.rowHasScope(token, 'issue.delete.epic.jira')) {
                    ...
                    this.addEpic(upload, parentEpicIssue);
                    if (BreakdownParse.rowHasScope(token, 'issue.key.jira')) {
                    ...
                    this.addStoryKeyToEpic(upload, parentEpicIssue, issue);
                    if (BreakdownParse.rowHasScope(token, 'issue.key.jira')) {
                        upload.uploadJiraObjects.push({
                            method: 'DELETE',
                            issue: issue,
        ...
        maintainEpicLink(storyIssue, parentIssue, settings) {
            if (BreakdownUtil.getKey(parentIssue)) {
                BreakdownUtil.setEpicLink(storyIssue, BreakdownUtil.getKey(parentIssue), settings);
            }
        },
