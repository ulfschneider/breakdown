...
export default BreakdownPush = (function () {
    ...
    return {
        ...
        makeUploadDescription(token, issue, method) {
            let description = BreakdownParse.parseDescription(token);
            BreakdownUtil.addDescription(issue, description, method);
            return description;
        },
        ...
        maintainDefaultFieldSettings(issue) {
            let settings = BreakdownEnvironment.getSettings();
            if (settings.fixversion && !BreakdownUtil.getFixVersions(issue).length) {
                BreakdownUtil.setFixVersion(issue, settings.fixversion);
            }
            if (settings.points && !BreakdownUtil.getStoryPoints(issue)) {
        ...
        makeUploadDataStructure() {
            ...
            tokens.forEach((token, rowCount) => {
                ...
                if (BreakdownParse.rowHasScope(token, 'issue.no-parent-in-selection.bkdn')) {
                ...
                } else if (BreakdownParse.rowHasScope(token, 'issue.description.bkdn')) {
                    let parentIssue;
                    ...
                    this.makeUploadDescription(token, parentIssue);
                }
            });
        },
