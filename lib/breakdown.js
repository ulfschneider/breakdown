...
export default {
    ...
    pullFromJIRA() {
        return BreakdownPull.pullFromJIRA(this.download, this.credentials)
            ...
            .catch((error) => {
                ...
                atom.notifications.addWarning('Pulled JIRA data could not be stringified' + (rootCause ? ' - ' + rootCause : ''), {
                    dismissable: true
                })
            });
    },
    ...
    pushToJIRA() {
        return BreakdownPush.pushToJIRA(this.upload, this.credentials)
            .then(() => {
                this.settings.stringifyAfterPull = true;
                this.pullFromJIRA();
            });
