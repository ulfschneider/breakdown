    ...
    return {
                ...
                .then((body) => {
                    ...
                    if (startAt + maxResults < total) {
                        ...
                        if (download.settings.stringifyAfterPull) {
                            BreakdownStatus.setStatus('Pulling from JIRA ' + BreakdownStringify.stringifyProgress(startAt, total));
                        }
                        ...
                        if (download.settings.stringifyAfterPull) {
                            BreakdownStatus.setStatus('Pulling from JIRA ' + BreakdownStringify.stringifyProgress(total, total));
                        }
        ...
        pullFromJira(download, credentials) {
            ...
            return this.pullCustomFieldNames(download.settings, credentials)
                ...
                .catch((error) => {
                    ...
                    if (download.settings.stringifyAfterPull) {
                        console.error('Failure when pulling JIRA' + (rootCause ? ' - ' + rootCause : ''));
                        atom.notifications.addWarning('Failure when pulling JIRA' + (rootCause ? '\n\n' + rootCause : ''), {
                            dismissable: true
                        });
                    } else {
        ...
        extractDataFromDownloadedIssues(download) {
            ...
            for (let issue of download.issues) {
                ...
                if (BreakdownUtils.isEpicType(issue, download.settings)) {
                ...
                } else if (BreakdownUtils.isStoryType(issue, download.settings)) {
                    download.statistics.storyCount++;
                    download.stories.set(issue.key, {
                        issue: issue,
                        subTasks: new Map(),
                    });
                } else if (BreakdownUtils.isSubTaskType(issue, download.settings)) {
        ...
        makeDownloadStoryDataStructure(download) {
            for (let story of download.stories.values()) {
                let epicKey = BreakdownUtils.getEpicLink(story.issue, download.settings);
                if (!epicKey) {
                    epicKey = 'NO PARENT';
                }
                ...
                download.epics.get(epicKey)
                    .stories.set(BreakdownUtils.getKey(story.issue), story);
            }
        },

        makeDownloadSubTaskDataStructure(download) {
            for (let subTask of download.subTasks.values()) {
                let parentKey = BreakdownUtils.getParentKey(subTask.issue);
                if (parentKey) {
                    let story = download.stories.get(parentKey);
                    if (story) {
                        subTask.parentIssue = story.issue;
                        story.subTasks.set(BreakdownUtils.getKey(subTask.issue), subTask);
                    } else {
                        let epic = download.epics.get(parentKey);
                        if (epic) {
                            subTask.parentIssue = epic.issue;
                            epic.subTasks.set(BreakdownUtils.getKey(subTask.issue), subTask);
                        }
                    }
                } else {
                    delete subTask.parentIssue;
                    download.epics.get('NO PARENT')
                        .subTasks.set(BreakdownUtils.getKey(subTask.issue), subTask);
                }
            }
        },
