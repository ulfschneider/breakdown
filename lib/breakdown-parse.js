...
export default BreakdownParse = (function () {
    return {
        ...
        readSettings() {
            ...
            if (!BreakdownEnvironment.isHandlingDifferentEditors()) {
                ...
                tokens.forEach((token, row) => {
                    ...
                    } else if (this.rowHasScope(token, 'settings.from-date.bkdn')) {
                        settings.fromDate = this.rowGetValueForScope(token, 'settings.value.bkdn');
                        settings.fromDate = BreakdownUtil.parseDate(settings.fromDate);
                        if (settings.fromDate) {
                            settings.fromDate = settings.fromDate.startOf('day');
                        }
                    } else if (this.rowHasScope(token, 'settings.to-date.bkdn')) {
                        settings.toDate = this.rowGetValueForScope(token, 'settings.value.bkdn');
                        settings.toDate = BreakdownUtil.parseDate(settings.toDate);
                        if (settings.toDate) {
                            settings.toDate = settings.toDate.startOf('day');
                        }
                    } else if (this.rowHasScope(token, 'settings.predict.bkdn')) {
                        settings.predict = this.rowGetValueForScope(token, 'settings.value.bkdn');
                        settings.predict = BreakdownUtil.parseDate(settings.predict);
                        if (settings.predict) {
                            settings.predict = settings.predict.startOf('day');
                        }
                    } else if (this.rowHasScope(token, 'settings.markers.bkdn')) {
                        settings.markers = this.parseMarkers(token);
        ...
        parseDescription(token) {
            return this.rowGetValuesForScope(token, 'description.value.bkdn');
        },
