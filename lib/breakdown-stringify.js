...
export default BreakdownStringify = (function() {
    ...
    return {
        ...
        stringifyJIRAIssue(jiraIssue, settings) {
            ...
            if (issue) {
                ...
                if (this.hasField(settings, 'parentkey')) {
                    let epic = settings.epicLinkFieldName ? issue.fields[settings.epicLinkFieldName] : '';
        ...
        stringifyJIRAContent(download) {
            ...
            for (let epic of download.epics.values()) {
                ...
                if (epic.issue) {
                    ...
                    totalPoints = this.summarizePoints(epic, download.settings);
                    donePoints = this.summarizeDonePoints(epic, download.settings);

                    result += 'NO EPIC IN SELECTION ';
                    result += this.stringifyProgress(donePoints, totalPoints);
                    result += '\n';
                }
        ...
        hasPoints(issue, settings) {
            return settings.storyPointsFieldName && this.hasField(settings, 'points') && issue.fields[settings.storyPointsFieldName];
        },
