...
import BreakdownStatusBar from './breakdown-status';
import {
    CompositeDisposable
} from 'atom';
...
export default {
        ...
        this.credentialsView = new BreakdownCredentialsView({
            submit: (user, pass) => {
                ...
                this.pull()
                    ...
                    .catch((error) => {
                        BreakdownStatusBar.clear();
                        console.log('Pulled JIRA data could not be stringified ' + BreakdownUtils.stringifyError(error));
                        atom.notifications.addError('Pulled JIRA data could not be stringified ' + BreakdownUtils.stringifyError(error), {
                            dismissable: true
                        })
                    });
    ...
    pullIssues(config) {
        ...
        if (_.isUndefined(config) || _.isUndefined(config.startAt) || config.startAt === 0) {
            BreakdownStatusBar.setStatus('Pulling JIRA issues ' + BreakdownStringify.stringifyProgress(0, 0));
        ...
        return this.request(options)
            .then((body) => {
                ...
                if (startAt + maxResults < total) {
                    ...
                    BreakdownStatusBar.setStatus('Pulling JIRA issues ' + BreakdownStringify.stringifyProgress(startAt, total));
                    config.startAt = startAt + maxResults;
                    config.maxResults = maxResults;
                    return this.pullIssues(config);
                } else {
                    BreakdownStatusBar.setStatus('Pulling JIRA issues ' + BreakdownStringify.stringifyProgress(total, total));
    ...
    pullCustomFieldNames() {
        console.log('Extracting custom field names');
        BreakdownStatusBar.setStatus('Extracting custom field names');
    ...
    pull() {
        ...
        BreakdownStatusBar.setStatus('Pulling JIRA data');
        ...
        return this.pullIssues({
            ...
            .then(() => {
                ...
                this.download.statistics.pulled = new Date();
