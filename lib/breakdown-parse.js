        ...
        readSettings(settings) {
            if (this.isBreakdownGrammar()) {
                ...
                tokens.forEach((token, row) => {
                    ...
                    } else if (BreakdownParse.rowHasScope(token, 'settings.pulled.jira') && BreakdownParse.rowHasScope(token, 'settings.value.jira')) {
                        settings.pulled = BreakdownParse.rowGetValuesForScope(token, 'settings.value.jira');
                    }
                });
                if (!hasUrl) {
                    settings.jiraUrl = '';
