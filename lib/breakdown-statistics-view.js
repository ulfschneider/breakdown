...
export default class BreakdownStatisticsView {
    ...
    render() {
        if (BreakdownEnvironment.getPath() == BreakdownEnvironment.getLastPullPath()) {
            //the order, cumulative flow first and duration second, is important, because settings are getting reused
    ...
    renderCumulativeFlowDiagram() {
        let settings = {};
        this.prepareCumulativeFlowDiagram(settings);
        ...
        this.appendCumulativeFlowUnresolved(settings);
    ...
    prepareCumulativeFlowDiagram(settings) {
    ...
    appendCumulativeFlowUnresolved(settings) {
        let download = BreakdownEnvironment.getDownload();
        let unresolved = 0;
        if (BreakdownUtil.hasOption(BreakdownUtil.OPTION_CFD_POINTS)) {
            unresolved = download.statistics.totalPoints - download.statistics.donePoints;
        } else {
            unresolved = download.assignees.issueCount() - download.assignees.resolvedIssueCount()
        }
        if (unresolved) {
