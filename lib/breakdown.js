...
export default {
    ...
    pull() {
        ...
        this.pullIssues({
            ...
            .then(() => {
                editor.insertText(BreakdownStringify.stringifyJIRAContent(this.download));
                console.log('JIRA data has been pulled\n' + BreakdownStringify.stringifyDownloadStatistics(this.download));
                atom.notifications.addInfo('JIRA data has been pulled');
                BreakdownBeautify.beautify(this.settings);
            })
            .catch((error) => {
                console.error('Failure when pulling JIRA data. ' + (error ? error.stack : ''));
                atom.notifications.addError('Failure when pulling JIRA data. ' + (error ? error.stack : ''), {
                    dismissable: true
                });
            });
    },
