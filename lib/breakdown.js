...
export default {
    ...
    activate() {
        ...
        atom.workspace.activePaneContainer.paneContainer.element.addEventListener('click', (event) => {
            BreakdownRequest.openIssue(event, this.settings);
        });
    ...
    pullFromJIRA() {
        return BreakdownPull.pull(this.download, this.credentials)
            .then((success) => {
            ...
            .catch((error) => {
                ...
                atom.notifications.addError('Pulled JIRA data could not be stringified' + (rootCause ? ' - ' + rootCause : ''), {
                    dismissable: true
                })
            });
    },
    ...
    pushToJIRA() {
        BreakdownPull.pullCustomFieldNames(this.settings, this.credentials)
            .then(() => {
                ...
                atom.notifications.addError('Data could not be prepared for pushing to JIRA' + (rootCause ? ' - ' + rootCause : ''), {
                    dismissable: true
                });
            });
    },
