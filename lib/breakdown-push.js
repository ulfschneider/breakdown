    ...
    return {
        ...
        hasPushLimit(settings) {
            return BreakdownUtil.hasOption(settings, 'pushdelete') ||
                ...
                BreakdownUtil.hasOption(settings, 'pushupdateself') ||
        ...
        isNoPushUpdate(updateJiraObject, settings, credentials) {
            return this.hasPushLimit(settings) &&
                !BreakdownUtil.hasOption(settings, 'pushupdate') &&
                !this.isPushUpdateSelf(updateJiraObject, settings, credentials);
        },

        isPushUpdateSelf(updateJiraObject, settings, credentials) {
            if (BreakdownUtil.hasOption(settings, 'pushupdateself')) {
        ...
        deleteIssue(deleteJiraObject, upload, credentials) {
            ...
            if (this.isNoPushDelete(upload.settings)) {
                ...
                console.log('Issue key=[' + BreakdownUtil.getKey(issue) + '] could not be deleted in JIRA due to missing pushdelete option');
        ...
        updateIssue(updateJiraObject, upload, credentials) {
            ...
            if (this.isNoPushUpdate(updateJiraObject, upload.settings, credentials)) {
                upload.statistics.failed.update.push(BreakdownUtil.createIssueMarkupLink(BreakdownUtil.getKey(issue), settings) + ' due to missing **pushupdate** or unfulfilled **pushupdateself** option');
                console.log('Issue key=[' + BreakdownUtil.getKey(issue) + '] could not be updated in JIRA due to missing pushupdate or unfulfilled pushupdateself option');
                return Promise.resolve();
            }
        ...
        createIssue(uploadJiraObject, upload, credentials) {
            ...
            if (this.isNoPushCreate(upload.settings)) {
                ...
                console.log('Issue issuetype=[' + BreakdownUtil.getIssueType(issue) + '] summary=[' + BreakdownUtil.getSummary(issue) + '] could not be created in JIRA due to missing pushcreate option');
                return Promise.resolve();
            }
