        ...
        readSettings() {
            ...
            if (BreakdownEnvironment.getEditor()) {
                ...
                tokens.forEach((token, row) => {
                    ...
                    } else if (BreakdownParse.rowHasScope(token, 'settings.markers.bkdn')) {
                        let markers = BreakdownParse.rowGetValuesForScope(token, 'marker.full.bkdn');
                        settings.markers = _.map(markers, marker => {
                            let m = {};
                            let delPos = marker.indexOf(':');
                            m.date = BreakdownUtil.parseDate(marker.substring(0, delPos));
                            m.label = BreakdownUtil.trim(marker.substring(delPos + 1));
                            return m;
                        });
                        markers = BreakdownParse.rowGetValuesForScope(token, 'marker.date.bkdn');
                        settings.markers = settings.markers.concat(_.map(markers, marker => {
                                return {
