        ...
        readSettings() {
            ...
            if (BreakdownEnvironment.getEditor()) {
                ...
                tokens.forEach((token, row) => {
                    if (BreakdownParse.rowHasScope(token, 'settings.jira-url.bkdn') && BreakdownParse.rowHasScope(token, 'settings.value.bkdn')) {
                    ...
                    } else if (BreakdownParse.rowHasScope(token, 'settings.marker.bkdn')) {
                        let markers = BreakdownParse.rowGetValuesForScope(token, 'marker.full.bkdn');
                        settings.marker = _.map(markers, marker => {
                            let m = {};
                        ...
                        settings.marker = settings.marker.concat(_.map(markers, marker => {
                                return {
