...
function isDescriptionField(token) {
	return BreakdownParse.rowHasScope(token, 'issue.description.field.bkdn');
}

function isAcceptanceField(token) {
	return BreakdownParse.rowHasScope(token, 'issue.acceptance.field.bkdn');
}

export default BreakdownBeautify = (function () {
	return {
		beautify({ readSettings, trim }) {
			...
			if (editor) {
				...
				let lines = BreakdownEnvironment.getLines();

				let grammar = BreakdownEnvironment.getGrammar();
				let lastDescriptionRow;
				let indentDescription = 0;
				let text = '';
				let rowCount = 0;
				for (let line of lines) {
					let token = grammar.tokenizeLine(line).tokens;
					if (BreakdownParse.rowHasScope(token, 'issue.no-parent-in-selection.bkdn') ||
						...
						isEpic(token) || isStory(token) || isSubTask(token) || isDescription(token) || isDescriptionField(token) || isAcceptanceField(token)
					) {
						let isFirstLineOfDescription = (rowCount - 1) != lastDescriptionRow;

						text += this.beautifyRow(token, {
							rowCount: rowCount,
							indentLevelOfDescription: indentDescription + (isFirstLineOfDescription || isDescriptionField(token) || isAcceptanceField(token) ? 0 : 1)
						});
						text += lineEnding;

						if (isDescription(token) || isDescriptionField(token) || isAcceptanceField(token)) {
					...
					} else if (!trim || (trim && BreakdownParse.rowGetValue(token).trim())) {
		...
		beautifyWhenUnlocked() {
			if (BreakdownEnvironment.isUnlocked()) {
				this.beautify({ readSettings: true });
			}
		},
		...
		beautifyRow(token, options) {
			...
			for (let i = 0; i < token.length; i++) {
				...
				} else if (BreakdownParse.tokenHasScope(t, 'issue.description.field.bkdn')) {
					value = BreakdownStringify.stringifyDescriptionField(token);
					row = ''.padStart(options.indentLevelOfDescription, '\t') + value;
				} else if (BreakdownParse.tokenHasScope(t, 'issue.acceptance.field.bkdn')) {
					value = BreakdownStringify.stringifyAcceptanceField(token);
					row = ''.padStart(options.indentLevelOfDescription, '\t') + value;
