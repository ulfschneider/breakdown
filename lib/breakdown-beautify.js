...
export default BreakdownBeautify = (function() {
    return {
            ...
            if (BreakdownParse.isBreakdownGrammar()) {
                ...
                tokens.forEach((token, rowCount) => {
                    if (BreakdownParse.rowHasScope(token, 'issue.epic.jira') ||
                        ...
                        text += this.beautifyRow(token, rowCount, resolvedRows, settings);
                        text += '\n';
                    } else {
                ...
                this.markResolvedIssues(resolvedRows);
                editor.setCursorScreenPosition(cursorPosition);
            }
        },

        markResolvedIssues(resolvedRows) {
            const editor = atom.workspace.getActiveTextEditor();

            //destroy all markers
            let markers = editor.getMarkers();
        ...
        calcSettingsValueIndent(settings) {
            if (settings.project) {
                return 8;
            } else if (settings.fields || settings.pulled || settings.pushed) {
                return 7;
            } else {
                return 6;
            }
        ...
        isResolvedStatus(status, issueType, settings) {
            if (BreakdownUtils.isSameIssueType(issueType, settings.epicType)) {
                return settings.epicResolvedStates.has(status);
            } else if (BreakdownUtils.isSameIssueType(issueType, settings.storyType)) {
                return settings.storyResolvedStates.has(status);
            } else if (BreakdownUtils.isSameIssueType(issueType, settings.subTaskType)) {
                return settings.subTaskResolvedStates.has(status);
            }
            return false;
        },

        beautifyRow(token, rowCount, resolvedRows, settings) {
            let row = '';
            ...
            token.forEach((t, i) => {
                ...
                if (BreakdownParse.tokenHasScope(t, 'issue.type.epic.jira')) {
                    ...
                    if (this.isResolvedStatus(value, issueType, settings)) {
                        resolvedRows.push(rowCount);
                    }
                    row += value;
