    ...
    return {
        ...
        httpRequest(options) {
            return new Promise(async (resolve, reject) => {
                let config = this.prepareOptions(options); //clone and prepare
                let url = config.url;
                delete config.url;
                try {
                    ...
                    if ((response.status >= 200 && response.status <= 208)) {
                        ...
                        if (response.status == 401) {
                            ...
                            let parsedUrl = BreakdownUtil.parseUrl(response.url);
                            let jiraUrl = parsedUrl.protocol + '//' + parsedUrl.hostname;
                            explanation += ` with the URL ${response.url}. `;
