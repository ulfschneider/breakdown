...
export default class TransitionLog {
    ...
    transitionsToJSON({
        ...
        json.columns = ['date'];
        json.columns = json.columns.concat([...this.doneStates].sort((a, b) => {
            return a.localeCompare(b);
        }));
        json.columns = json.columns.concat([...this.indeterminateStates].sort((a, b) => {
            return a.localeCompare(b);
        }));
        json.columns = json.columns.concat([...this.newStates].sort((a, b) => {
            return a.localeCompare(b);
        }));
    ...
    addIssue(issue) {
        //this.transitions = SoMap(date => SoMap(status => [issues]))
        //this.resolvedIssues = []
        ...
        issueTransitions.forEach((status, date) => {
            ...
            if (!this.transitions.has(date)) {
                this.transitions.set(date, new SoMap(null, (a, b) => {
            ...
            if (!statusForDay.has(status)) {
                ...
                let statusCategory = BreakdownUtil.getStatusCategory(status);
                this.maintainStates(status, statusCategory);
            }
