...
export default BreakdownPush = (function() {
    ...
    return {
        ...
        getIssueDiff(originIssue, modifiedIssue, settings) {
            ...
            if (BreakdownUtils.isEpicType(modifiedIssue, settings)) {
        ...
        pushIssues(upload, credentials) {
            ...
            return upload.newJiraObjects.reduce((series, uploadJiraObject, count) => {
                    return series.then(() => {
                        BreakdownStatus.setStatus('Create issues in JIRA ' + BreakdownStringify.stringifyProgress(count, upload.newJiraObjects.length));
                        return this.createIssue(uploadJiraObject, upload.settings, credentials)
                    });
                }, Promise.resolve())
                .then(() => {
                    return upload.updateJiraObjects.reduce((series, updateJiraObject, count) => {
                        return series.then(() => {
                            BreakdownStatus.setStatus('Update issues in JIRA ' + BreakdownStringify.stringifyProgress(count, upload.updateJiraObjects.length));
                            return this.updateIssue(updateJiraObject, upload.settings, credentials)
                        });
                    }, Promise.resolve())
                })
                .then(() => {
                    return upload.deleteJiraObjects.reduce((series, deleteJiraObject, count) => {
                        return series.then(() => {
                            BreakdownStatus.setStatus('Delete issues from JIRA ' + BreakdownStringify.stringifyProgress(count, upload.deleteJiraObjects.length));
