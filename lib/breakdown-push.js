...
export default BreakdownPush = (function() {
    ...
    return {
        ...
        deleteIssue(deleteJiraObject, settings, credentials) {
            ...
            return BreakdownRequest.request(options)
                .catch((error) => {
                    ...
                    console.error('Issue could not be deleted in JIRA, issuetype=[' + BreakdownUtil.getIssueType(issue) + '] summary=[' + BreakdownUtil.getSummary(issue) + ']' + (rootCause ? ' - ' + rootCause : ''));
        ...
        updateIssue(updateJiraObject, settings, credentials) {
            ...
            let status = BreakdownUtil.getStatus(issue); //we cannot set status directly
            BreakdownUtil.deleteStatus(issue);
            ...
            return BreakdownRequest.request(options)
                ...
                .catch((error) => {
                    let rootCause = BreakdownUtil.stringifyError(error);
                    console.error('Issue could not be updated in JIRA, key=[' + BreakdownUtil.getKey(issue) + ']' + (rootCause ? ' - ' + rootCause : ''));
        ...
        createIssue(uploadJiraObject, settings, credentials) {
            ...
            return BreakdownRequest.request(options)
                .then((body) => {
                    ...
                    let rootCause = BreakdownUtil.stringifyError(error);
                    console.error('Issue could not be created in JIRA, issuetype=[' + BreakdownUtil.getIssueType(issue) + '] summary=[' + BreakdownUtil.getSummary(issue) + ']' + (rootCause ? ' - ' + rootCause : ''));
