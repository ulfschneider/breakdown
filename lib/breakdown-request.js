...
export default BreakdownRequest = (function() {
    ...
    return {
                ...
                request(options, (error, response, body) => {
                    ...
                    if (error && error.code) {
                        ...
                        if (response.statusCode == 401) {
                            BreakdownUtil.clear(options.credentials);
        ...
        prepareOptions(config, settings, credentials) {
            ...
            if (config.url) {
                ...
                options.url = settings.jiraUrl +
                    ...
                    (config.maxResults ? '&maxResults=' + config.maxResults : '&maxResults=500') +
                    '&fields=*all';
            } else {
        ...
        openJira(event, settings, credentials) {
            if ((event.ctrlKey || event.metaKey) && BreakdownParse.isBreakdownGrammar()) {
                destroyCursor(event);
                BreakdownParse.readSettings(settings, credentials);
                if (!settings.jiraUrl) {
