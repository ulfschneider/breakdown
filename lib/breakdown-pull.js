...
export default BreakdownPull = (function() {
  ...
  return {
    ...
    pullIssues(config, credentials, download) {
      let options = BreakdownRequest.prepareOptions(config, credentials, download.settings);
      ...
      return BreakdownRequest.request(options)
        .then((body) => {
          ...
          if (startAt + maxResults < total) {
            ...
            return this.pullIssues(config, credentials, download);
          } else {
    ...
    pullCustomFieldNamesWithKey(key, credentials, settings) {
      if (key) {
    ...
    pullCustomFieldNames(credentials, download) {
      ...
      return this.pullCustomFieldNamesWithKey(issueKey, credentials, download.settings);
    },


    pull(credentials, download) {
      ...
      return this.pullIssues({
        ...
        .then(() => {
          return this.pullCustomFieldNames(credentials, download);
        })
        .then(() => {
