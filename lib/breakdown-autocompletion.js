...
export default class BreakdownAutocompletionProvider {
    constructor() {
        ...
        this.selector = '.bkdn, .breakdown, breakdown';
    ...
    prepareAutocompletion(reduced) {
        if (BreakdownUtil.isFalsy(reduced) && BreakdownUtil.isFalsy(BreakdownEnvironment.getSettings()
                .project)) {
            return false;
        }
        if (BreakdownEnvironment.getSettings()
            .jiraUrl) {
    ...
    isOffline() {
        ...
        return BreakdownUtil.hasOption(BreakdownUtil.OPTION_OFFLINE);
    }
    ...
    pullComponents(query) {
        if (BreakdownUtil.isFalsyOrEmpty(this.components) &&
            ...
            let options = BreakdownRequest.prepareOptions({
                url: BreakdownEnvironment.getSettings()
                    .jiraUrl + '/rest/api/2/project/' + BreakdownEnvironment.getSettings()
                    .project + '/components',
            });
            BreakdownStatusBar.pull('Fetching components');
    ...
    pullProjects(query) {
        if (BreakdownUtil.isFalsyOrEmpty(this.projects) &&
            ...
            BreakdownStatusBar.pull('Fetching projects');
            return BreakdownRequest.request(options)
                .then(body => {
    ...
    pullVersions(query) {
        if (BreakdownUtil.isFalsyOrEmpty(this.versions) &&
            ...
            BreakdownStatusBar.pull('Fetching versions');
            return BreakdownRequest.request(options)
                .then(body => {
    ...
    pullUsers(query) {
        if (query &&
            ...
            let options = BreakdownRequest.prepareOptions({
                url: BreakdownEnvironment.getSettings()
                    .jiraUrl + '/rest/api/2/user/picker?query=' + query,
            });
            BreakdownStatusBar.pull('Fetching users');
            return BreakdownRequest.request(options)
                .then(body => {
    ...
    pullTransitions(key, query) {
        ...
        if (BreakdownUtil.isFalsyOrEmpty(transitions) &&
            ...
            let options = BreakdownRequest.prepareOptions({
                url: BreakdownEnvironment.getSettings()
                    .jiraUrl + '/rest/api/2/issue/' + key + '/transitions',
            });
            BreakdownStatusBar.pull('Fetching transitions');
            return BreakdownRequest.request(options)
                .then(body => {
    ...
    getBreakdownOptions(query) {
        return this.mapSuggestions(['create', 'update', 'updateself', 'delete', 'rank', 'parentkey', 'nopush', 'offline', 'cfdpoints'], query);
    }

    getBreakdownSettings(query) {
        return this.mapSuggestions(['url: ', 'project: ', 'query: ', 'options: ', 'fixversion: ', 'points: ', 'fromdate: ', 'todate: ', 'markers: ', 'predict: '], query);
    }
    ...
    getSuggestions({
        ...
        if (this.isCreateSettings(scopeDescriptor) && bufferPosition.row > 0) {
            ...
            if (BreakdownParse.tokenCountScopes(scopeDescriptor) == 1 && bufferPosition.row == 0) {
                ...
                return this.mapSuggestions(['WITHOUT PARENT', 'Epic', 'Story', 'Sub-task', '//'], prefix);
            }
        }
        return null;
    }
