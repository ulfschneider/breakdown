...
export default BreakdownUtil = (function () {
    ...
    return {
        ...
        EDITABLE_CONTENT: 'Below this line is the editable breakdown content',
        RESOLVED: 'Resolved',
        DELETE: 'DEL',
        ACTIVE_SPRINT: '⟳ ',
        ...
        OPTION_UPDATE_SELF: 'updateself',
        OPTION_NOPUSH: 'nopush',
        OPTION_RANK: 'rank',
        OPTION_OFFLINE: 'offline',
        OPTION_CFD_POINTS: 'cfdpoints',
        PUSH_GUARDS: ['create', 'delete', 'update', 'updateself', 'nopush'],
        DEFAULT_PUSH_GUARDS: ['create', 'updateself'],
        DAY_FORMAT: 'dddd D-MMM HH:mm[, GMT]Z',
        ...
        formatDuration(duration) {
            ...
            if (duration.asMonths() <= 3 && duration.asDays() >= 1) {
                result += Math.floor(duration.asDays()) + 'd'
            }
            if (duration.asDays() < 1 && duration.hours() >= 1) {
                result += duration.hours() + 'hh';
            }
            if (duration.asDays() < 1 && duration.minutes() >= 1) {
                result += duration.minutes() + 'mm';
            }
        ...
        progressBar(fraction) {
            ...
            const progress = ['⭘', '◔', '◑', '◕', '⬤'];
            const completion = progress[Math.max(0, Math.min(Math.floor((progress.length - 1) * fraction), progress.length - 1))];

            return `${completion} ${Math.round(fraction * 100)}%`;
        },
        ...
        stringifySeconds(seconds) {
            ...
            const hoursPerDay = BreakdownEnvironment.getWorkingHoursPerDay();
            const daysPerWeek = BreakdownEnvironment.getWorkingDaysPerWeek();

            const SECOND = 1;
            const MINUTE = SECOND * 60;
            const HOUR = MINUTE * 60;
            const DAY = HOUR * hoursPerDay;
            const WEEK = DAY * daysPerWeek;
            let result = '';
            if (seconds) {
            ...
            if (settings.sprintFieldName) {
                ...
                if (this.isTruethy(sprint)) {
                    return this.isTruethy(_.find(sprint, data => {
                        return data.state == 'active' || data.indexOf && data.indexOf('state=ACTIVE') >= 0;
                    }));
                }
            }
            return false;
        ...
        getDescription(issue) {
            return this.splitLines(issue.fields.description);
        },

        getAcceptanceCriteria(issue) {
            let settings = BreakdownEnvironment.getSettings();
            let value = settings.acceptanceCriteriaFieldName && issue.fields[settings.acceptanceCriteriaFieldName] ? issue.fields[settings.acceptanceCriteriaFieldName] : '';
            return value ? this.splitLines(value) : [];
        },
        ...
        getOriginalEstimate(issue) {
            return issue.fields.timetracking && issue.fields.timetracking.originalEstimate ? issue.fields.timetracking.originalEstimate : '';
        },

        getOriginalEstimateSeconds(issue) {
            return issue.fields.timetracking && issue.fields.timetracking.originalEstimateSeconds ? issue.fields.timetracking.originalEstimateSeconds : 0;
        },

        getAggregateOriginalEstimateSeconds(issue) {
            return issue.fields.aggregatetimeoriginalestimate ? issue.fields.aggregatetimeoriginalestimate : 0;
        },
        ...
        setDescription(issue, description) {
            description = this.joinLines(description);
        ...
        setAcceptanceCriteria(issue, description, origIssue) {
            let settings = BreakdownEnvironment.getSettings();
            let issueType = this.getIssueType(issue) || this.getIssueType(origIssue)

            if (issueType == 'Epic' || issueType == 'Story') {
                description = this.joinLines(description);

                if (settings.acceptanceCriteriaFieldName) {
                    issue.fields[settings.acceptanceCriteriaFieldName] = description;
                }
            } else {
                this.setDescription(issue, description);
            }
        },

        addAcceptanceCriteria(issue, description, origIssue) {
            let settings = BreakdownEnvironment.getSettings();
            let issueType = this.getIssueType(issue) || this.getIssueType(origIssue)

            if (issueType == 'Epic' || issueType == 'Story') {
                description = this.joinLines(description);
                if (description && settings.acceptanceCriteriaFieldName) {
                    if (issue.fields[settings.acceptanceCriteriaFieldName]) {
                        issue.fields[settings.acceptanceCriteriaFieldName] += '\n\n' + description;
                    } else {
                        issue.fields[settings.acceptanceCriteriaFieldName] = description;
                    }
                }
            } else {
                this.addDescription(issue, description);
            }
        },
        ...
        setOriginalEstimate(issue, origEstimate, method) {

            if (this.equalsIgnoreCase(method, this.METHOD_CREATE) && !origEstimate && !issue.fields.timetracking) {
                //do not touch timetracking in this case
                return;
            }
            ...
            if (origEstimate) {
                ...
                issue.fields.timetracking.originalEstimate = origEstimate;
            } else {
                if (issue.fields.timetracking) {
                    issue.fields.timetracking.originalEstimate = null;
                }
            }
        },
