                ...
                request(options, (error, response, body) => {
                    if (!error && (response.statusCode >= 200 && response.statusCode <= 208)) {
                        ...
                        if (response.statusCode == 401) {
                            ...
                            explanation = '. If the problem persists and you are sure to use the correct username and passphrase, try to sign out and in to [JIRA](' + jiraUrl + ') with your web browser. It might be you need to answer a [CAPTCHA](https://en.wikipedia.org/wiki/CAPTCHA) to access [JIRA](' + jiraUrl + ') again. Another reason for not getting access could be, your Jira server only allows REST API access with an API Token Authentication. You can create an access token from your Jira profile menu (at the top right corner of your Jira window) and use that token as a passphrase when working with breakdown (instead your usual passphrase).';
                        }
