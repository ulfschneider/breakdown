...
export default class BreakdownAutocompletionProvider {
    ...
    prepareAutocompletion(reduced) {
        ...
        if (BreakdownEnvironment.getSettings()
            ...
            if (BreakdownUtil.isFalsy(credentials.user) ||
    ...
    getBreakdownOptions(query) {
        return this.mapSuggestions(['create', 'update', 'updateself', 'delete', 'rank', 'parentkey', 'nopush', 'offline', 'statistics'], query);
    }
    ...
    getSuggestions({
        ...
        if (this.isCreateSettings(scopeDescriptor) && bufferPosition.row > 0) {
            ...
            if (BreakdownParse.tokenCountScopes(scopeDescriptor) == 1 && bufferPosition.row == 0) {
                ...
                const suggestions = ['WITHOUT PARENT', 'Epic', 'Story', 'Sub-task', 'Acceptance', 'Description'];
                let prefixRegEx = new RegExp(`^${prefix}`, 'i');
                return this.mapSuggestions(suggestions.filter(text => prefixRegEx.test(text)));
            }
        }
        return null;
    }
