    ...
    return {
        ...
        displayStatistics() {
            ...
            if (upload.uploadJiraObjects.length || upload.rankingObjects.length) {
                success = BreakdownUtil.formatDate(upload.statistics.pushed) + '<br>Data has been pushed from **' + BreakdownEnvironment.getFile() + '** to JIRA';
            } else {
        ...
        isPushingDesired() {
            return !BreakdownUtil.hasOption(BreakdownUtil.NO_PUSH);
        },
        ...
        makeUploadDataStructure() {
            ...
            tokens.forEach((token, rowCount) => {
                ...
                if (BreakdownParse.rowHasScope(token, 'issue.no-parent-in-selection.bkdn')) {
                    ...
                    issue = this.makeUploadEpicIssue(token, BreakdownUtil.METHOD_DELETE);
                    parentEpicIssue = issue;
                    parentStoryIssue = null;
                    ...
                    BreakdownParse.rowHasScope(token, 'issue.resolved.epic.bkdn')) {
                    issue = this.makeUploadEpicIssue(token, method);
                    parentEpicIssue = issue;
                    parentStoryIssue = null;
                    BreakdownRank.addEpic(parentEpicIssue);
                    if (method == BreakdownUtil.METHOD_CREATE) {
                        this.maintainDefaultFieldSettings(issue);
                        upload.uploadJiraObjects.push({
                            method: BreakdownUtil.METHOD_CREATE,
                            issue: issue
                        });
                    } else if (originIssue) {
                        let diffIssue = this.getIssueDiff(originIssue, issue, parentEpicIssue);
                    ...
                    issue = this.makeUploadStoryIssue(token, BreakdownUtil.METHOD_DELETE);
                    ...
                    BreakdownParse.rowHasScope(token, 'issue.resolved.story.bkdn')) {
                    issue = this.makeUploadStoryIssue(token, parentEpicIssue, method);
                    parentStoryIssue = issue;
                    BreakdownRank.addStoryToEpic(parentEpicIssue, issue);
                    ...
                    issue = this.makeUploadSubTaskIssue(token, parentIssue, BreakdownUtil.METHOD_DELETE);
                    if (BreakdownParse.rowHasScope(token, 'issue.key.bkdn')) {
                        upload.uploadJiraObjects.push({
                            method: BreakdownUtil.METHOD_DELETE,
                            issue: issue,
                    ...
                    BreakdownParse.rowHasScope(token, 'issue.resolved.sub-task.bkdn')) {
                    let parentIssue = (parentStoryIssue ? parentStoryIssue : parentEpicIssue);
                    issue = this.makeUploadSubTaskIssue(token, parentIssue, method);
                    if (method == BreakdownUtil.METHOD_CREATE) {
                        upload.uploadJiraObjects.push({
                            method: BreakdownUtil.METHOD_CREATE,
                            issue: issue,
        ...
        hasPushLimit() {
            return BreakdownUtil.hasOption(BreakdownUtil.PUSH_DELETE) ||
                BreakdownUtil.hasOption(BreakdownUtil.PUSH_UPDATE) ||
                BreakdownUtil.hasOption(BreakdownUtil.PUSH_UPDATE_SELF) ||
                BreakdownUtil.hasOption(BreakdownUtil.PUSH_CREATE);
        },

        isNoDeleteDesired() {
            return !BreakdownUtil.hasOption(BreakdownUtil.PUSH_DELETE) && this.hasPushLimit();
        },
        ...
        isUpdateSelf(updateJiraObject) {
            if (BreakdownUtil.hasOption(BreakdownUtil.PUSH_UPDATE_SELF)) {
        ...
        isNoCreateDesired() {
            return !BreakdownUtil.hasOption(BreakdownUtil.PUSH_CREATE) && this.hasPushLimit();
        },
