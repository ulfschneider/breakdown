    ...
    return {
            ...
            return BreakdownRequest.request(options)
                .then(body => {
                    ...
                    if (startAt + maxResults < total) {
                        ...
                        BreakdownStatusBar.pull('Pulling from JIRA ' + BreakdownStringify.stringifyProgress(startAt + maxResults, total));
                        config.startAt = startAt + maxResults;
                        config.maxResults = maxResults;
                        return this.pullIssues(config);
                    } else {
                        BreakdownStatusBar.pull('Pulling from JIRA ' + BreakdownStringify.stringifyProgress(total, total));
        ...
        pullFromJira() {
            ...
            if (BreakdownEnvironment.getMode() == BreakdownEnvironment.MODE_PUSH) {
                BreakdownStatusBar.pull('Prepare pushing to JIRA');
            } else {
                BreakdownStatusBar.pull('Pulling from JIRA');
            ...
            return this.pullConfiguration()
                ...
                .then(() => {
                    return this.pullIssues({
                        ...
                        allFields: BreakdownEnvironment.getMode() == BreakdownEnvironment.MODE_PULL,
                        changelog: true
                    });
                })
                .then(() => {
                ...
                .then(() => {
                    ...
                    if (BreakdownEnvironment.getMode() == BreakdownEnvironment.MODE_PULL) {
                        atom.notifications.addSuccess(BreakdownUtil.formatDate(download.statistics.pulled) + '\n\nData has been pulled from JIRA into **' + BreakdownEnvironment.getFile() + '**', {
                            dismissable: true
                        });
                    } else if (BreakdownEnvironment.getMode() == BreakdownEnvironment.MODE_PULL_CHART) {
                        atom.notifications.addSuccess(BreakdownUtil.formatDate(download.statistics.pulled) + '\n\nData has been pulled from JIRA into the **chart panel** of **' + BreakdownEnvironment.getFile() + '**', {
                            dismissable: true
                        });
                    }
        ...
        extractDataFromDownloadedIssues() {
            ...
            download.transitionLog = new TransitionLog();
            ...
            for (let issue of download.issues) {
                download.transitionLog.addIssue(issue);
