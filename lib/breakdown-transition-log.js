...
export default class TransitionLog {
    ...
    transitionsToJSON({
        ...
        this.transitionLog().forEach((statusMap, date) => {
            today = moment(date);
    ...
    durationsToJSON({
        ...
        let json = [];
        this.durationLog().forEach((resolvedAtDate, date) => {
            today = moment(date);
            if ((BreakdownUtil.isFalsy(fromDate) || today.isSameOrAfter(fromDate)) &&
                (BreakdownUtil.isFalsy(toDate) || today.isSameOrBefore(toDate))) {
                json.push({
                    date: today,
                    avgCycleTime: resolvedAtDate.avgCycleTime,
                    avgLeadTime: resolvedAtDate.avgLeadTime
                });
            }
        });
        json.columns = ['date', 'avgCycleTime', 'avgLeadTime'];
    ...
    getTransitionLogForIssue(issue) {
        ...
        if (BreakdownUtil.isResolved(issue)) {
            issue.leadTime = moment.duration(moment(BreakdownUtil.getResolvedDate(issue)).diff(issue.fields.created));
            if (cycleStart) {
                issue.cycleTime = moment.duration(moment(BreakdownUtil.getResolvedDate(issue)).diff(cycleStart));
            }
        }
