...
export default BreakdownPush = (function() {
    ...
    return {
        ...
        hasPushLimit(settings) {
            return BreakdownUtil.hasOption(settings, 'pushdelete') ||
                BreakdownUtil.hasOption(settings, 'pushupdate') ||
                BreakdownUtil.hasOption(settings, 'pushcreate');
        },

        isNoPushDelete(settings) {
            return !BreakdownUtil.hasOption(settings, 'pushdelete') && this.hasPushLimit(settings);
        },

        isNoPushUpdate(settings) {
            return !BreakdownUtil.hasOption(settings, 'pushupdate') && this.hasPushLimit(settings);
        },

        isNoPushCreate(settings) {
            return !BreakdownUtil.hasOption(settings, 'pushcreate') && this.hasPushLimit(settings);
        },
        ...
        deleteIssue(deleteJiraObject, upload, credentials) {
            ...
            if (this.isNoPushDelete(upload.settings)) {
                upload.statistics.failed.delete.push(BreakdownUtil.createIssueMarkupLink(BreakdownUtil.getKey(issue), settings) + ' due to missing **pushdelete** option');
                console.log('Issue could not be deleted in JIRA, key=[' + BreakdownUtil.getKey(issue) + '] due to missing pushdelete option');
            ...
            return BreakdownRequest.request(options)
                ...
                .catch(error => {
                    ...
                    console.log('Issue could not be deleted in JIRA, issuetype=[' + BreakdownUtil.getIssueType(issue) + ']' + (rootCause ? ' due to ' + rootCause : ''));
                });
        },
        ...
        updateIssue(updateJiraObject, upload, credentials) {
            ...
            if (this.isNoPushUpdate(upload.settings)) {
                upload.statistics.failed.update.push(BreakdownUtil.createIssueMarkupLink(BreakdownUtil.getKey(issue), settings) + ' due to missing **pushupdate** option');
                console.log('Issue could not be updated in JIRA, key=[' + BreakdownUtil.getKey(issue) + '] due to missing pushupdate option');
                return Promise.resolve();
            }
            ...
            return BreakdownRequest.request(options)
                ...
                .catch(error => {
                    ...
                    console.log('Issue could not be updated in JIRA, key=[' + BreakdownUtil.getKey(issue) + ']' + (rootCause ? ' due to ' + rootCause : ''));
                });

        },

        createIssue(uploadJiraObject, upload, credentials) {
            let settings = upload.settings;
            this.maintainParent(uploadJiraObject, settings);
            let issue = uploadJiraObject.issue;

            if (this.isNoPushCreate(upload.settings)) {
                upload.statistics.failed.create.push(BreakdownUtil.getIssueType(issue) + ' ' + BreakdownUtil.getSummary(issue) + ' due to missing **pushcreate** option');
                console.log('Issue could not be created in JIRA, issuetype=[' + BreakdownUtil.getIssueType(issue) + '] summary=[' + BreakdownUtil.getSummary(issue) + '] due to missing pushcreate option');
                return Promise.resolve();
            }
            ...
            return BreakdownRequest.request(options)
                ...
                .catch(error => {
                    ...
                    console.log('Issue could not be created in JIRA, issuetype=[' + BreakdownUtil.getIssueType(issue) + '] summary=[' + BreakdownUtil.getSummary(issue) + ']' + (rootCause ? ' due to ' + rootCause : ''));
                });
        },
