                ...
                this.pull()
                    .then(() => {
                        if (this.settings.stringifyAfterPull) {
                            BreakdownStringify.stringifyAfterPull(download, settings);
                        }
                    })
                    .catch((error) => {
        ...
        return new Promise((resolve, reject) => {
            ...
            .then((issueKey) => {
                if (issueKey) {
                    ...
                    return this.request(options)
                        .then((body) => {
                            BreakdownPull.extractCustomFieldNames(body, settings);
                        });
                }
                console.warn('Custom field names could not be extracted');
                resolve(); //return with resolve, as there might be no problem with not having the custom field names
            });
    },
