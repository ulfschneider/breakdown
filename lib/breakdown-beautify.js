...
export default BreakdownBeautify = (function () {
    return {
                ...
                if (tokens.length) {
                    ...
                    tokens.forEach((token, rowCount) => {
                        if (BreakdownParse.rowHasScope(token, 'issue.no-parent-in-selection.bkdn') ||
                            ...
                            BreakdownParse.rowHasScope(token, 'issue.description.bkdn') ||
                            BreakdownParse.rowHasScope(token, 'settings.bkdn')) {

                            text += this.beautifyRow(token, rowCount, (rowCount - 1) != lastDescriptionRow);

                            if (BreakdownParse.rowHasScope(token, 'issue.description.bkdn')) {
        ...
        beautifyRow(token, rowCount, firstLineOfDescription) {
            let row = '';
