    ...
    return {
        ...
        stringifyUnresolvedIssues(unresolvedIssues, settings) {
            ...
            unresolvedIssues.forEach((issue, index) => {
                ...
                result += ', ';
                result += BreakdownUtil.getStatus(issue);
                let remainingEstimate = BreakdownUtil.stringifySeconds(BreakdownUtil.getRemainingEstimateSeconds(issue),
        ...
        stringifyDownloadStatistics(download) {
            ...
            if (assigneeCount) {
                ...
                let remainingEstimate = BreakdownUtil.stringifySeconds(download.assignees.getRemainingEstimateSeconds(),
                    this.getWorkingHoursPerDay(settings),
                    this.getWorkingDaysPerWeek(settings));
                ...
                download.assignees
                    .forEach(assignee => {
                        ...
                        let remainingEstimate = BreakdownUtil.stringifySeconds(assignee.getRemainingEstimateSeconds(),
                            this.getWorkingHoursPerDay(settings),
                            this.getWorkingDaysPerWeek(settings));
