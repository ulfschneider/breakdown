...
//TODO limit to start and finish date -> in config
//TODO button to load chart -> in panel
//TODO decide for points or issue count -> in panel + default in config
...
export default class BreakdownChartView {
    ...
    render() {
        ...
        layer.filter(function(d) {
                return true;
                y(d[d.length - 1][0]) - y(d[d.length - 1][1]) >= 10; //10 = font size in px
            })
            .append("text")
            .attr("x", width + 40)
            .attr("y", function(d) {
