...
export default BreakdownPush = (function() {
    ...
    return {
        ...
        isPushingDesired(settings) {
            return !BreakdownUtil.hasOption(settings, BreakdownUtil.NO_PUSH);
        },
        ...
        hasPushLimit(settings) {
            return BreakdownUtil.hasOption(settings, BreakdownUtil.PUSH_DELETE) ||
                BreakdownUtil.hasOption(settings, BreakdownUtil.PUSH_UPDATE) ||
                BreakdownUtil.hasOption(settings, BreakdownUtil.PUSH_UPDATE_SELF) ||
                BreakdownUtil.hasOption(settings, BreakdownUtil.PUSH_CREATE);
        },

        isNoDeleteDesired(settings) {
            return !BreakdownUtil.hasOption(settings, BreakdownUtil.PUSH_DELETE) && this.hasPushLimit(settings);
        },
        ...
        isUpdateSelf(updateJiraObject, settings, credentials) {
            if (BreakdownUtil.hasOption(settings, BreakdownUtil.PUSH_UPDATE_SELF)) {
        ...
        isNoCreateDesired(settings) {
            return !BreakdownUtil.hasOption(settings, BreakdownUtil.PUSH_CREATE) && this.hasPushLimit(settings);
        },
        ...
        updateIssue(updateJiraObject, upload, credentials) {
            ...
            if (this.isNoUpdateDesired(updateJiraObject, upload.settings, credentials)) {
                upload.statistics.failed.update.push(BreakdownUtil.createIssueMarkupLink(BreakdownUtil.getKey(issue), settings) + ' due to missing **update** option or unfulfilled **updateself** option');
