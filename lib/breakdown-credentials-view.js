...
export default class BreakdownCredentialsView {
    ...
    constructor(config) {
        ...
        this.element.addEventListener('keydown', this.escCallback = event => {
            if (event.which == 27) {
        ...
        this.user.addEventListener('keydown', this.userTabCallback = event => {
            if (event.which == 9) {
                event.preventDefault();
                event.stopImmediatePropagation();
                this.pass.focus();
        ...
        this.pass.addEventListener('keydown', this.passTabCallback = event => {
            if (event.which == 9) {
                event.preventDefault();
                event.stopImmediatePropagation();
                this.user.focus();
    ...
    destroy() {
        this.element.removeEventListener('keydown', this.escCallback);
        this.user.removeEventListener('keydown', this.userTabCallback);
        this.pass.removeEventListener('keydown', this.passTabCallback);
        this.element.remove();
    }

    getUser() {
        return BreakdownUtil.trim(this.user.value);
    }
