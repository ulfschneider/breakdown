...
export default {
    ...
    pullIssues(config) {
        ...
        if (_.isUndefined(config) || _.isUndefined(config.startAt) || config.startAt === 0) {
            BreakdownStatus.setStatus('Pulling JIRA issues ' + BreakdownStringify.stringifyProgress(0, 0));
    ...
    pullCustomFieldNames() {
        ...
        BreakdownStatus.setStatus('Extracting custom field names');
    ...
    initiatePush() {
        ...
        if (BreakdownParse.isBreakdownGrammar()) {
            ...
            if (editor) {
                ...
                BreakdownPush.makeUploadDataStructure(this.upload);
            }
