    ...
    return {
            ...
            return BreakdownRequest.request(options)
                .then(body => {
                    ...
                    if (startAt + maxResults < total) {
                        ...
                        BreakdownStatusBar.pull('Pulling from JIRA ' + BreakdownStringify.stringifyProgress(startAt + maxResults, total) + ' into ' + BreakdownEnvironment.getFileNameWithoutExtension());
                        config.startAt = startAt + maxResults;
                        config.maxResults = maxResults;
                        return this.pullIssues(config);
                    } else {
                        BreakdownStatusBar.pull('Pulling from JIRA ' + BreakdownStringify.stringifyProgress(total, total) + ' into ' + BreakdownEnvironment.getFileNameWithoutExtension());
        ...
        pullFromJira() {
            ...
            if (BreakdownEnvironment.getMode() == BreakdownEnvironment.MODE_PUSH) {
                BreakdownStatusBar.pull('Prepare pushing to JIRA from ' + BreakdownEnvironment.getFileNameWithoutExtension());
            } else {
                BreakdownStatusBar.pull('Pulling from JIRA into ' + BreakdownEnvironment.getFileNameWithoutExtension());
