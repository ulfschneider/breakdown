    ...
    return {
        ...
        stringifyJiraObject(jiraObject, settings) {
            ...
            if (issue) {
                ...
                result += BreakdownUtils.getIssueType(issue);
                result += ' ';
                result += issue.key
                result += ' ';
                ...
                if (this.hasField(settings, 'parentkey')) {
                    let epic = BreakdownUtils.getEpicLink(issue, settings);
                    let parent = BreakdownUtils.getParentKey(issue);
                    if (epic) {
                        result += ' ^';
                        result += epic;
                    } else if (parent) {
                ...
                result += BreakdownUtils.getEditorLineEnding();
            }
            return result;
        },
        ...
        stringifyFields(jiraObject, settings) {
            ...
            if (BreakdownUtils.isEpicType(issue, settings)) {
            ...
            if (this.hasPrimaryFields(issue, settings) ||
                ...
                if (this.hasPoints(issue, settings) || totalPoints) {
                    result += ' p:';
                    let points = BreakdownUtils.getStoryPoints(issue, settings);
                    if (points) {
                        result += points;
                    }
                    if (totalPoints) {
                ...
                if (this.hasFixVersion(issue, settings)) {
                    let fixversions = BreakdownUtils.getFixVersions(issue);
                    result += ' v:';
                    result += BreakdownUtils.stringifyFixVersions(fixversions);
                }
        ...
        stringifySettings(download) {
            let result = '';
            let lineEnding = BreakdownUtils.getEditorLineEnding();
            result += 'breakdown';
            result += lineEnding;
        ...
        hasPoints(issue, settings) {
            return this.hasField(settings, 'points') && BreakdownUtils.getStoryPoints(issue, settings);
        },
        ...
        summarizePoints(jiraObject, settings, onlyDonePoints) {
            ...
            if (issue && this.hasPoints(issue, settings)) {
                if (BreakdownUtils.isFalseOrUndefined(onlyDonePoints) ||
                    ...
                    result += BreakdownUtils.getStoryPoints(issue, settings);
                }
            }
